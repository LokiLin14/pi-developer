...
2. If plan approval is absent, including when the user only agreed to a short conversational proposal, do not implement or apply that agreement retroactively. Create/update a task only if needed; always write/update the concrete plan document as `State: PROPOSED`, reusing relevant records. If the plan is vague, stale, or blocked by untested assumptions, refine it or recommend `/skill:grill` or `/skill:check`. Present the full plan and stop for explicit approval of that version before setting `APPROVED`. This gate applies without a slash command too; once the presented concrete plan is explicitly approved, proceed without another planning round.
...
6. Inspect the completed diff against every planned step. If a material deviation is necessary, stop, revise the plan as `PROPOSED`, present the full revised plan, and obtain renewed explicit approval before continuing. Do not broaden scope to fix unrelated failures.
...
10. Perform a final scope audit and `git diff --check`. Mark the plan `COMPLETE` and any linked task checks done only when supported by evidence. Missing required validation or reviews must remain visible as blockers, not fabricated approvals.
