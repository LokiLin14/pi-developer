...
5. If the user only requested preparation, return the draft and proposed commands. Before committing, pushing, or opening a PR, confirm authorization for those actions and the target remote/base. When authorized, stage explicit task-owned paths after inspecting their contents; do not use blanket staging that could capture user work. Never force-push or bypass hooks/checks.
