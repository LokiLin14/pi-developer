...
1. Inspect repository contribution rules, PR templates (including `.github/` when present), branch/commit conventions, remotes, current branch, working tree, and relevant existing decision/task/plan records. Determine the intended base branch; do not assume it is `main`. Infer the Git destination/action from clear context; ask only if materially ambiguous, distinguishing a local merge from remote publication. Do not switch branches or disturb unrelated work without permission.
2. Inspect the entire proposed PR diff and commit history against the base (normally using the merge base / three-dot diff), not just uncommitted changes; check prerequisite commits against the intended remote base before promising a single-purpose diff, since local main may contain commits absent there. Confirm that it serves one concrete goal, contains no secrets or scratch artifacts, and includes only authorized changes. If it mixes unrelated goals, stop and propose a split.
...
7. At handoff, state the actual local/remote result: local main, remote main, a pushed work branch, and PR merge status as relevant; if published, include the actual PR URL and CI status, distinguishing pending from passed. Do not merge unless explicitly requested, required checks pass, and approvals/branch policies permit it. Do not delete branches without permission.
