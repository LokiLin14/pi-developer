...
Even without a slash command, agreement to a short conversational proposal approves direction only. Create/update a task only when needed; settle unresolved material choices before writing/updating the concrete plan, then present the full document and wait for explicit approval of that version before building. Drafting needs no extra approval; straightforward corrections can be edited directly. See [.pi/APPEND_SYSTEM.md](.pi/APPEND_SYSTEM.md) and [grill](.pi/skills/grill/SKILL.md) for details. Reuse relevant documents; approval of the presented plan permits implementation without another planning round.
